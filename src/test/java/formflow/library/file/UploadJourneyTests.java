    ...
    void documentUploadFlowMultiFlow() {
        ...
        await().until(() -> {
            try {
                ...
                return !title.isBlank() && title.equals("Upload Documents");
            } catch (Exception e) {
                return false;
            }
        });
        ...
        // Wait a bit for navigation to fully complete before going back
        await().until(() -> {
            try {
                String title = testPage.getTitle();
                return !title.isBlank() && !title.equals("Upload Documents");
        ...
        // First wait for title to be available
        await().until(() -> {
            try {
                String title = testPage.getTitle();
                return !title.isBlank() && title.equals("Upload Documents");
        ...
        // Wait for the dropzone widget to be initialized and have files loaded
        await().until(() -> {
            try {
                // Check if dropzone element exists
                List<WebElement> dropzoneElements = driver.findElements(By.id("document-upload-" + dzWidgetName));
                if (dropzoneElements.isEmpty()) {
                    return false;
                }
                // Check if dropzone JavaScript object exists and has files
                // The dropzone is stored as window['myDropZone' + inputName]
                Object dropzoneFiles = driver.executeScript(
                    "return window['myDropZone" + dzWidgetName + "'] && window['myDropZone" + dzWidgetName + "'].files ? window['myDropZone" + dzWidgetName + "'].files.length : 0");
                if (dropzoneFiles == null) {
                    return false;
                }
                Long fileCount = ((Number) dropzoneFiles).longValue();
                return fileCount == 5;
            } catch (Exception e) {
                return false;
            }
        });
        // Wait for the file count indicator element to exist and show files are loaded
        await().until(() -> {
            try {
                List<WebElement> fileCountElements = driver.findElements(By.id("number-of-uploaded-files-" + dzWidgetName));
                if (fileCountElements.isEmpty()) {
                    return false;
                }
                String fileCount = fileCountElements.get(0).getText();
                return fileCount.equals("5 files added");
            } catch (Exception e) {
                return false;
            }
        });
        ...
        await().until(() -> {
            try {
                ...
                return !elements.isEmpty() && elements.size() == 5;
            } catch (Exception e) {
                return false;
            }
        });
        ...
        driver.navigate().to(baseUrl);
        // Wait for page to load - first wait for title
        await().until(() -> {
            try {
                String title = testPage.getTitle();
                return !title.isBlank() && title.equals("Upload Documents");
        ...
        // Wait for the dropzone widget to be initialized
        await().until(() -> {
            try {
                List<WebElement> dropzoneElements = driver.findElements(By.id("document-upload-" + dzWidgetName));
                return !dropzoneElements.isEmpty();
            } catch (Exception e) {
                return false;
            }
        });
