...
public class Page {
  ...
  public void goBack() {
    ...
    // Wait for navigation to complete - URL should change or page should reload
    await().until(() -> {
      try {
        String newUrl = driver.getCurrentUrl();
        return !newUrl.equals(currentUrl) || !driver.getTitle().isBlank();
      } catch (Exception e) {
        return false;
      }
    });
  ...
  public void clickContinue() {
    ...
    // Wait for navigation to complete - either URL changes or main-footer updates
    await().until(() -> {
      try {
        String newUrl = driver.getCurrentUrl();
        boolean urlChanged = !newUrl.equals(currentUrl);
        boolean footerUpdated = !driver.findElements(By.className("main-footer")).isEmpty() &&
            !driver.findElements(By.className("main-footer")).get(0).getDomProperty("innerHTML").isBlank();
        return urlChanged || footerUpdated;
      } catch (Exception e) {
        return false;
      }
    });
