...
public class SeleniumFactory implements FactoryBean<RemoteWebDriver> {
  ...
  public void start() {
    ...
    //WebDriverManager.chromedriver().browserVersion(WebDriverManager.chromedriver().getDownloadedDriverVersion());
    ...
    if (browserPath.isPresent()) {
      WebDriverManager
          .chromedriver()
          .browserVersionDetectionCommand(browserPath.get() + " -- version")
          .setup();
    } else {
      // will grab the latest version if the binary is not found on system
