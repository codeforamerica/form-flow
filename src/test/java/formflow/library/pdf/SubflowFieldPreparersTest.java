...
class SubflowFieldPreparersTest {
    ...
    void shouldNotCreateFieldsForIncompleteSubflowIterations() {
        ...
        assertThat(resultMap.equals(
                Map.of(
                        "foo_1",
                        new SingleField("foo", "foo from first iteration will not be removed because iteration is complete", 1),
                        "bar_1",
                        new SingleField("bar", "bar from first iteration will not be removed because iteration is complete", 1),
                        "foo_2",
                        new SingleField("foo", "foo from third iteration will not be removed because iteration is complete", 2),
                        "bar_2",
                        new SingleField("bar", "bar from third iteration will not be removed because iteration is complete", 2)
                )
        )).isTrue();
    }
