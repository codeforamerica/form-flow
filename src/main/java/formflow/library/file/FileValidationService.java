...
public class FileValidationService {
    ...
    private final Map<String, Set<MimeType>> FILE_EXT_MIME_TYPE_MAP = Map.ofEntries(
            ...
            Map.entry(".docx", Set.of(new MimeType("application", "vnd.openxmlformats-officedocument.wordprocessingml.document"),
                    new MimeType("application", "x-tika-ooxml"))),
    ...
    public Boolean isAcceptedMimeType(MultipartFile file) throws IOException {
        ...
        if (ZIP_MIME_TYPE.equals(mimeType) && ACCEPTED_FILE_EXTS.contains(".docx") &&
            ...
            try (InputStream inputStream = file.getInputStream(); ZipInputStream zipInputStream = new ZipInputStream(
                    inputStream)) {
                ZipEntry entry;
