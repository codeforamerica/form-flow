...
/**
 * Parses the flow configuration yaml file and adds validated flow configuration objects to the libraries list of flow
 * configurations.
 */
@Slf4j
public class FlowsConfigurationFactory implements FactoryBean<List<FlowConfiguration>> {
     ...
     * FlowConfiguration objects. Disabled flows will be excluded, detected using the form-flow.disabled-flows application
     * property.
     *
    ...
    public List<FlowConfiguration> getObject() throws IOException {
        ...
        try {
            ...
            flowConfigsIterable.forEach(
                    flowConfig -> addValidatedFlowConfiguration((FlowConfiguration) flowConfig, flowConfigurations));
        } catch (IOException e) {
    ...
    private void validateFlowConfiguration(FlowConfiguration flowConfig) {
        if (formFlowConfigurationProperties != null && formFlowConfigurationProperties.isSubmissionLockedForFlow(
                flowConfig.getName())) {
    ...
    protected void validateLandmarksFirstScreen(FlowConfiguration flowConfig) {
        if (flowConfig.getLandmarks() == null || flowConfig.getLandmarks().getFirstScreen() == null) {
            throw new FlowConfigurationException(
                    "You have enabled the session continuity interceptor in your application but have not added a first screen landmark for the flow "
                            + flowConfig.getName() +
